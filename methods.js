...
function getAllNames(array) {
  return array.map((item) => console.log(item.name));
}
...
function removeAddresses(array) {
  return array.map(({ address, ...rest }) => rest);
}
...
function getUsersGeo(array, id) {
  return array.find((item) => (item.id === id ? item.address.geo : false));
}
...
function getUsersCompany(array, id) {
  return array.find((item) => (item.id === id ? item : null));
}
...
function changePhone(array, id, phone) {
  return array.filter((item) => (item.id === id ? (item.phone = phone) : null));
}
